...
export default function ProjectDetailToc({ toc }: { toc: TocItem[] }) {
  return (
    <motion.div
        ...
        <div className="h-full overflow-y-scroll overflow-x-hidden flex flex-col gap-4 p-5 pb-24">
          {toc.map((item) => (
            <a
              ...
              className="text-sm text-muted-foreground hover:text-foreground transition cursor-pointer block h-fit"
            >
              {item.text}
            </a>
          ))}
